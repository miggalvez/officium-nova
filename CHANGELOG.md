...
**Validation.** Per design §19.1, the authority order is Ordo Recitandi → governing rubrical books (1911 / 1955 / 1960) → legacy Divinum Officium Perl output. `composeHour()` runs exception-free across every Hour for every date in 2024 under each of the three Roman policies (8,784 compositions per run) via the `test:no-throw` integration test. The live Perl comparison harness at `pnpm -C packages/compositor compare:phase-3-perl` now surfaces a `Matching prefix` metric per row and per-policy best/average summaries — forward progress is visible even when row counts stay flat. Current ledger state is: Divino Afflatu `496` divergent hours with `456` still unadjudicated (best/average matching prefix `4/2.9` lines), Reduced 1955 `488` with `458` unadjudicated (`126/30.6`), and Rubrics 1960 `488` with `451` unadjudicated (`126/31.0`). Classifications live in [`packages/compositor/test/divergence/adjudications.json`](packages/compositor/test/divergence/adjudications.json) per [ADR-011](docs/adr/011-phase-3-divergence-adjudication.md), with chronological pattern resolutions in [`packages/compositor/test/divergence/ADJUDICATION_LOG.md`](packages/compositor/test/divergence/ADJUDICATION_LOG.md) and upstream `perl-bug` families tracked in [`docs/upstream-issues.md`](docs/upstream-issues.md).

Latest 3h burn-down work closed the Roman Vespers later-block checkpoint and the Jan `14` `Rubrics 1960` minor-hour later-block checkpoint without broadening scope. On the code side, `applyRuleSet()` now falls back to the Sunday `Minor Special` chapter / responsory / versicle bundle for Jan `14` `1960` `Terce` / `Sext` / `None` when the Ordinarium wrapper would otherwise leave those slots empty, and the upstream tests now lock the exact Vespers psalm refs plus the Jan `14` post-psalmody order. On the adjudication side, the shared Roman Vespers heading families are now classified as source-backed `perl-bug`s: Jan `1/7` Vespers keeps `Psalmus 110 [2]` from the Day0 table, and the Epiphany / Baptism family keeps the fifth-slot Psalm `116` override instead of Day0 Psalm `113`. The remaining Jan `14` `1960` minor-hour difference is also now classified as `perl-bug`: Perl inserts underscore separator lines around the short responsory even though `Minor Special.txt` does not.
