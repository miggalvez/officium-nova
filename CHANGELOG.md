...
**Validation.** Per design §19.1, the authority order is Ordo Recitandi → governing rubrical books (1911 / 1955 / 1960) → legacy Divinum Officium Perl output. `composeHour()` runs exception-free across every Hour for every date in 2024 under each of the three Roman policies (8,784 compositions per run) via the `test:no-throw` integration test. The live Perl comparison harness at `pnpm -C packages/compositor compare:phase-3-perl` now surfaces a `Matching prefix` metric per row and per-policy best/average summaries — forward progress is visible even when row counts stay flat. Current ledger state is: Divino Afflatu `496` divergent hours with `456` still unadjudicated (best/average matching prefix `4/2.9` lines), Reduced 1955 `488` with `462` unadjudicated (`126/30.6`), and Rubrics 1960 `488` with `457` unadjudicated (`126/30.8`). Classifications live in [`packages/compositor/test/divergence/adjudications.json`](packages/compositor/test/divergence/adjudications.json) per [ADR-011](docs/adr/011-phase-3-divergence-adjudication.md), with chronological pattern resolutions in [`packages/compositor/test/divergence/ADJUDICATION_LOG.md`](packages/compositor/test/divergence/ADJUDICATION_LOG.md) and upstream `perl-bug` families tracked in [`docs/upstream-issues.md`](docs/upstream-issues.md).

Latest 3h burn-down work tightened the non-Matins Roman psalter surface without reopening Phase 2: `Tridentinum#antiphon` now materializes the Jan `14` Sunday minor-hour antiphons from the keyed `[Prima]` / `[Tertia]` / `[Sexta]` / `[Nona]` psalter sections, generic psalmody now preserves repeated explicit closes while still reopening the next wrapper antiphon, and psalmody normalization strips numeric carry markers such as `(5)` / `(8)`. The live compare frontier moved accordingly: the Jan `14` Rubrics 1960 generic-`Allelúja` minor-hour family and the `(8)` Vespers carry-marker family disappeared as code bugs, while the remaining Jan `14` Sunday psalter-surface rows and the shared Psalm 99 `‡ ... *` Lauds family are now tracked as adjudicated `perl-bug` families.
